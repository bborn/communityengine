...
xm.item do
  xm.title "#{h(post.respond_to?(:topic_title) ? post.topic_title : post.topic.title)} (#{h post.topic.sb_posts_count == 1 ? '' : :reply.l } #{:by.l} #{h post.username} on #{post.created_at.to_s(:short)})"
