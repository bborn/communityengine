...
class RsvpsControllerTest < ActionController::TestCase
  ...
  def test_should_update_rsvp
    login_as :quentin
    put :update, :event_id => "2", :id => rsvps(:quentin_rsvp_for_future_event).id, :rsvp => {:attendees_count => '3' }
    assert_redirected_to event_path(events(:future_event))
  end

  def test_should_fail_to_update_rsvp
    login_as :quentin
    put :update, :event_id => "2", :id => rsvps(:quentin_rsvp_for_future_event).id, :rsvp => { :attendees_count => nil }
