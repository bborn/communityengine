...
class FavoritesControllerTest < ActionController::TestCase
  ...
  def test_should_create_favorite_as_logged_in_user
    ...
    assert_difference Favorite, :count, 1 do
      xhr :post, :create, :favoritable_type => 'clipping', :favoritable_id => clippings(:google).id, :format => :js
    end
  ...
  def test_should_destroy_favorite
    ...
    assert_difference Favorite, :count, -1 do
      xhr :delete, :destroy, :favoritable_type => 'clipping', :favoritable_id => clippings(:google).id, :id => favorite.id, :format => :js
      assert_response :success
    end
  end
