...
class PostTest < ActiveSupport::TestCase
  ...
  def test_should_find_post_by_featured_writers
    featured = []
    2.times do
      featured << users(:super_writer).posts.
        create(:raw_post => 'Blog post message', :title => 'Title')
    end
    featured.each(&:save_as_live)

    assert_equal Post.find_featured, featured.reverse
  end
end
